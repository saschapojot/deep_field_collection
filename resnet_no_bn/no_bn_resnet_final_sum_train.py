...
from no_bn_resnet_structure_final_sum import format_using_decimal, no_bn_resnet_final_sum,CustomDataset
if (len(sys.argv) != 5):
...
model=no_bn_resnet_final_sum(num_spins=num_spins,num_layers=num_layers,num_neurons=num_neurons).double().to(device)
...
# Save the loss log
with open(out_model_Dir + f"/no_bn_resnet_final_sum_training_log.txt", "w+") as fptr:
    fptr.writelines(loss_file_content)


# Save the model
torch.save(model.state_dict(), out_model_Dir + f"/no_bn_resnet_final_sum_model.pth")
