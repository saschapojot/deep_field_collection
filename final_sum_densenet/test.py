...
dirs_root=f"./out_model_final_sum_densenet_L{L}_K{K}_r{r}_layer{num_layers}/"
for dir in glob.glob(dirs_root+f"/growth_rate*"):
    match_neuron = re.search(r"growth_rate(\d+)", dir)
    if match_neuron:
...
def load_one_model_performance(num_neurons,epoch_num,dir):
    model_file_name = dir + f"/final_sum_densenet_trained_epoch{epoch_num}.pth"
    model=densenet_final_sum(
        num_spins=L, num_layers=num_layers, growth_rate=num_neurons
    )
