...
out_model_Dir=f"./out_model_final_sum_densenet_L{L}_K{K}_r{r}_layer{num_layers}/growth_rate{growth_rate}/"
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % save_interval == 0:
        save_path = out_model_Dir + f"/final_sum_densenet_trained_epoch{epoch}.pth"
        checkpoint = {
            'epoch': epoch + 1,
...
# Save the model
torch.save(model.state_dict(), out_model_Dir + f"final_sum_densenet_model.pth")
