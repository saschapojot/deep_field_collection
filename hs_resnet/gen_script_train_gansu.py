...
def contents_to_bash(num_layers,num_neurons,file_index):
    ...
    contents = [
        ...
        "#SBATCH -p lzicnormal\n",
        "#SBATCH --mem=20GB\n",
        f"#SBATCH -o out_densenet_no_bn_layer{num_layers}_neuron{num_neurons}.out\n",
        f"#SBATCH -e out_densenet_no_bn_layer{num_layers}_neuron{num_neurons}.err\n",
        "cd /public/home/hkust_jwliu_1/liuxi/Documents/pyCode/deep_field_collection/hs_resnet\n",
        f"python3 -u hs_resnet_train.py {L} {r} {num_layers} {num_neurons}\n",
        ]
    out_chunk = outPath + f"/chunk{file_index // chunk_size}/"
    Path(out_chunk).mkdir(exist_ok=True, parents=True)
    outBashName = out_chunk + f"/train_hs_resnet_layer{num_layers}_neuron{num_neurons}.sh"
