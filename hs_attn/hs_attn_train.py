...
out_model_Dir=f"./out_model_hs_attn_L{L}_K{K}_r{r}_layer{num_layers}/neuron{num_neurons}/"
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % save_interval == 0:
        ...
        log_path = out_model_Dir + f"/hs_attn_training_log.txt"
...
# Save the loss log
with open(out_model_Dir + f"/hs_attn_training_log.txt", "w+") as fptr:
    fptr.writelines(loss_file_content)

# Save the model
torch.save(model.state_dict(), out_model_Dir + f"hs_attn_model.pth")
