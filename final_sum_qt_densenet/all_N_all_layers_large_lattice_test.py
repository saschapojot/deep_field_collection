...
def evaluate_model(model, test_loader, device):
    ...
    with torch.no_grad():  # No need to compute gradients during evaluation
        for X_batch, Y_batch in test_loader:
            ...
            T1, F1, S1 = model.initialize_S1(X_batch)

            # Forward pass
            predictions = model(T1, F1, S1)
...
for N in N_vec:
    for layer in layer_all:
        ...
        in_model_file = in_model_dir + f"final_sum_qt_densenet_trained_over50_epoch{set_epoch}_num_samples200000.pth"
