...
def N_2_test_file(N,layer_num):
    ...
    test_txt_file = in_file_dir + f"/test_epoch{set_epoch}_num_samples{num_suffix}.txt"
...
#layer1
plt.scatter(N_vec,relative_error_layer1,color="blue",marker="o",s=marker_size1,label=f"densenet, n={layer1}")
plt.plot(N_vec,relative_error_layer1,color="blue",linestyle="dashed",linewidth=lineWidth1)


#layer 2
plt.scatter(N_vec,relative_error_layer2,color="magenta",marker="^",s=marker_size1,label=f"densenet, n={layer2}")
plt.plot(N_vec,relative_error_layer2,color="magenta",linestyle="dashed",linewidth=lineWidth1)


#layer 2
plt.scatter(N_vec,relative_error_layer3,color="green",marker="s",s=marker_size1,label=f"densenet, n={layer3}")
...
plt.savefig(out_db_qt_dir + f"/relative_error_C{C}_epoch{set_epoch}_pbc.png", bbox_inches="tight")  # bbox_inches ensures no truncation

plt.savefig(out_db_qt_dir + f"/relative_error_C{C}_epoch{set_epoch}_pbc.svg", bbox_inches="tight")  # bbox_inches ensures no truncation
plt.close()
