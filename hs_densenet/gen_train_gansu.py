...
outPath="./bashFiles_hs_densenet_no_bn/"
...
def contents_to_bash(num_layers,num_neurons_final_est,file_index):
    growth_rate=num_neurons_final_est
    print(f"num_layers={num_layers}, growth_rate={growth_rate}, final_neuron_num={num_neurons_final_est}")
    contents = [
        "#!/bin/bash\n",
        "#SBATCH -n 5\n",
        "#SBATCH -N 1\n",
        ...
        f"#SBATCH -e out_growth_rate{growth_rate}_densenet_no_bn_layer{num_layers}.err\n"
        "cd /public/home/hkust_jwliu_1/liuxi/Documents/pyCode/deep_field_collection/hs_densenet\n",
        f"python3 -u hs_densenet_train.py {L} {r} {num_layers}  {growth_rate}\n",
    ]
    out_chunk = outPath +f"/chunk{file_index // chunk_size}/"
    Path(out_chunk).mkdir(exist_ok=True, parents=True)
    outBashName = out_chunk + f"/train_growth_rate{growth_rate}_hs_densenet_layer{num_layers}.sh"
