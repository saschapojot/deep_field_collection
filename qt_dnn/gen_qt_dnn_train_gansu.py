...
#for training
#quantum Hamiltonian, pbc, dnn

outPath="./bashFiles_qt_dnn/"
...
def contents_to_bash(layer,C,file_index):
    contents = [
        ...
        "#SBATCH -p lzicnormal\n",
        "#SBATCH --mem=40GB\n",
        f"#SBATCH -o out_train_qt_dnn_layer{layer}_C{C}.out\n",
        f"#SBATCH -e out_train_qt_dnn_layer{layer}_C{C}.err\n",
        "cd /public/home/hkust_jwliu_1/liuxi/Documents/pyCode/deep_field_collection/qt_dnn\n",
        f"python3 -u qt_dnn_train.py {num_epochs} {C} {layer}\n",
        ]
    out_chunk = outPath + f"/chunk{file_index // chunk_size}/"
    Path(out_chunk).mkdir(exist_ok=True, parents=True)
    outBashName = out_chunk + f"/train_qt_dnn_layer{layer}_C{C}.sh"
