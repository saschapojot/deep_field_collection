...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % save_interval == 0:
        save_path = out_model_Dir + f"/hs_dnn_trained_epoch{epoch}.pth"
        checkpoint = {
            'epoch': epoch + 1,
...
# Save the loss log
with open(out_model_Dir + f"/hs_dnn_training_log.txt", "w+") as fptr:
    fptr.writelines(loss_file_content)


# Save the model
torch.save(model.state_dict(), out_model_Dir + f"hs_dnn_model.pth")
