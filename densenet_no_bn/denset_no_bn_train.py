...
from denset_no_bn_structure import format_using_decimal, DenseNet_no_bn,CustomDataset
...
num_skip_connections = int(sys.argv[3])
num_neurons=int(sys.argv[4])

num_layers=num_skip_connections+1
...
out_model_Dir=f"./out_model_densenet_no_bn_L{L}_K{K}_r{r}_layer{num_layers}/neuron{num_neurons}/"
...
# Save the loss log
with open(out_model_Dir + f"/densenet_no_bn_training_log.txt", "w+") as fptr:
    fptr.writelines(loss_file_content)

# Save the model
torch.save(model.state_dict(), out_model_Dir + f"/densenet_no_bn_model.pth")
