...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % save_interval == 0:
        save_path = out_model_Dir + f"/final_sum_hs_dnn_trained_epoch{epoch}.pth"
        checkpoint = {
            'epoch': epoch + 1,
...
# Save the model
torch.save(model.state_dict(), out_model_Dir + f"final_sum_hs_dnn_model.pth")
