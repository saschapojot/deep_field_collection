...
num_layers=3
name="hs_final_sum_densenet"
num_neurons_vec=[]
num_neurons_dirs_vec=[]

dirs_root=f"./out_model_hs_final_sum_densenet_L{L}_K{K}_r{r}_layer{num_layers}/"

for dir in glob.glob(dirs_root+f"/growth_rate*"):
    match_neuron_num=re.search(r"growth_rate(\d+)", dir)
    if match_neuron_num:
...
def load_one_log(dir):
    log_fileName=dir+"/training_log.txt"
