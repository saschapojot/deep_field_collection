...
outPath="./bashFiles_hs_final_sum_densenet/"
...
def contents_to_bash(num_layers,num_neurons,file_index):
    ...
    contents = [
        ...
        "#SBATCH -p lzicnormal\n",
        "#SBATCH --mem=20GB\n",
        f"#SBATCH -o out_hs_fs_densenet_layer{num_layers}_neuron{num_neurons}.out\n",
        f"#SBATCH -e out_hs_fs_densenet_layer{num_layers}_neuron{num_neurons}.err\n",
        "cd /public/home/hkust_jwliu_1/liuxi/Documents/pyCode/deep_field_collection/final_sum_hs_densenet\n",
        f"python3 -u train.py {L} {r} {num_layers} {num_neurons}\n"

    ]
    out_chunk = outPath + f"/chunk{file_index // chunk_size}/"
    Path(out_chunk).mkdir(exist_ok=True, parents=True)
    outBashName = out_chunk + f"/train_layer{num_layers}_neuron{num_neurons}.sh"
