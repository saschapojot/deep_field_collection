...
#for training
#resnet final sum
outPath="./bashFiles_resnet_final_sum/"
...
def contents_to_bash(num_layers,num_neurons,file_index):
    contents = [
        ...
        "#SBATCH -p lzicnormal\n",
        "#SBATCH --mem=40GB\n",
        f"#SBATCH -o out_train_resnet_final_sum_layer{num_layers}_neuron{num_neurons}.out\n",
        f"#SBATCH -e out_train_resnet_final_sum_layer{num_layers}_neuron{num_neurons}.err\n",
        "cd /public/home/hkust_jwliu_1/liuxi/Documents/pyCode/deep_field_collection/resnet\n",
        f"python3 -u resnet_final_sum_train.py {L} {r} {num_layers} {num_neurons}\n",
    ]

    out_chunk = outPath + f"/chunk{file_index // chunk_size}/"
    Path(out_chunk).mkdir(exist_ok=True, parents=True)
    outBashName = out_chunk + f"/train_resnet_final_sum_layer{num_layers}_neuron{num_neurons}.sh"
