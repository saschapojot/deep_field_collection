...
from growth_rate_denset_no_bn_structure import growth_rate_DenseNet_no_bn,CustomDataset
...
model=growth_rate_DenseNet_no_bn(
num_spins=num_spins,num_layers=num_layers,growth_rate=growth_rate
).double().to(device)
...
out_model_Dir=f"./out_model_densenet_no_bn_L{L}_K{K}_r{r}_layer{num_layers}/growth_rate{growth_rate}/"
...
# Save the loss log
with open(out_model_Dir + f"/growth_rate{growth_rate}_densenet_no_bn_training_log.txt", "w+") as fptr:
    fptr.writelines(loss_file_content)


# Save the model
torch.save(model.state_dict(), out_model_Dir + f"/growth_rate{growth_rate}_densenet_no_bn_model.pth")
