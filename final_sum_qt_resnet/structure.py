...
class final_sum_qt_resnet(nn.Module):
    def __init__(self, input_channels, phi0_out_channels, T_out_channels,
        ...
        # Tanh activation
        self.tanh = nn.Tanh()
        # for resnet, the symmetrization layers are only Phi0 and T1
        # Phi0Layer
    ...
    def initialize_S1(self, x):
        ...
        T1 = self.T1_layer(x)
        # standard resnet implementation: adding skip connection T1
        S1=T1+F1
        return S1
    ...
    def forward(self,x, Sn):
        for j in range(0, self.stepsAfterInit):
            ...
            # Step 2: next Sn=Sn+F_{n+1}
            Sn=Sn+Fn_plus_1

        # Step 3: Map the final Sn to N x N matrix
        Sn_padded = F.pad(Sn, (self.final_padding, self.final_padding, self.final_padding, self.final_padding),
                          mode='circular')
